...
const AutoDiscoverySection = ({
  ...
  const generateCommand = () => {
    ...
    let scriptUrl = `https://raw.githubusercontent.com/ctfox66/my-tz-kori/refs/heads/master/komari-agent/${scriptFile}`;
    if (enableGhproxy && ghproxy) {
    ...
    switch (selectedPlatform) {
      ...
      case "docker": {
        ...
        finalCommand =
          ...
          `ghcr.io/ctfox66/my-tz-kori-agent:latest ` +
...
function GenerateCommandButton({ node, settings }: { node: NodeDetail, settings: any }) {
  ...
  const generateCommand = () => {
    ...
    let scriptUrl =
      `https://raw.githubusercontent.com/ctfox66/my-tz-kori/refs/heads/master/komari-agent/${scriptFile}`;
    if (enableGhproxy) {
      if (enableGhproxy && ghproxy) {
        scriptUrl = scriptUrl.slice(8); // 去掉 https://
        if (ghproxy.endsWith("/")) {
